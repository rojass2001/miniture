...
function Kitchen() {
  ...
  <Box className='w-full mt-[90px] sm:mt-[150px] '>
    <Text className='text-3xl text-center font-bold text-black mb-4'> You  Are   In  Kitchen</Text>
    <Box className='w-full   grid grid-cols-4 gap-x-6 gap-y-3 sm:grid-cols-2 sm:gap-x-2 '>
      {kitchen?.map((a)=>(
        <Link to="/productdetail">
          <Box onClick={()=> dispatch(productdetailcontain(a))} className={`rounded-2xl group relative ${hover} h-[280px] sm:h-[200px] `} key={a.id}>
