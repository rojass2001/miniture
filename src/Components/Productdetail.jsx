...
function Productdetail() {
 ...
 const dispatch=useDispatch()
  return (
<>
    <div className='w-full h-screen place-content-center absolute top-1 justify-center     place-items-center sm:px-1'>
        ...
        <div className='h-full w-[40%] sm:w-full'>
          <img className='h-full w-full' src={productdetails?.image}/>
           ...
           <div className='w-[60%] sm:w-full sm:text-center sm:place-items-center font-bold text-black pt-3'>
              ...
              <p  className='text-[12px]'>{productdetails?.des}</p>
              <p className=' text-xl '>${productdetails?.price}</p>
              <p className='text-green-400 sm:text-[10px]'>{productdetails.stocks}</p>
              <div className='flex items-center '>
                <button onClick={()=>dispatch(decrease())} className='w-14 border-[1px] h-9 border-gray-300 mr-2'>-</button>
                {productdetails.quantity}
                <button onClick={()=>dispatch(increase())} className='w-14 border-[1px] h-9 border-gray-300 ml-2'>+</button>
              </div>
              <div className='flex gap-2 mt-4 sm:gap-4 '>
                <button onClick={()=>dispatch(addtocart(productdetails))} className='w-[120px] h-10 bg-orange-400 sm:w-[80px]'>Addtocart</button>
